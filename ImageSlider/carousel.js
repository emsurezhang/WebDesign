...
+function ($) {
  ...
  var Carousel = function (element, options) {
    ...
    //.carousel-inidicators 是Image页面的角标，页面上显示的小圆点。HTML的代码如下
    /**
     * <!-- Indicators -->
    ...
    //逻辑运算符 && 计算法过程，如果expr1为false则返回expr1；否则返回expr2。
    //expr1 判定为false的条件是:null undefined ±0 NaN false the empty string
    //这里的实际意义是代替if(判断条件) {执行语句} 为 判断条件 && 执行语句
    //当判断条件为true时，不能返回expr1，&&计算符检查expr2，从而执行&&后面的操作。不论expr2返回的是true或false，都不重要，因为目的是执行expr2
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    // 所以我们经常写的if( expr1 || expr2 )，这个操作 ||并不直接产生true、false的结果。而是根据expr1、expr2返回对应的表达式expr3。
    // 再由if(expr3)产生结果。
    // 对一个表达式expr1是否为true，javascript有如下判断规则:
    // 变量如果不为0，null，undefined，false，都会被处理为true;反之为false。
    // 所以 next作为一个函数传递参数，如果为空，则$next 被 this.getItemForDirection(type, $active)所替换。
